...
fn extract_seeds(input: &str) -> Vec<u128> {
    let mut seeds = vec![];
    let seed_values = input.trim().split_ascii_whitespace().filter_map(|number| {
        number.parse::<u128>().ok()
    }).collect::<Vec<u128>>();
    for value in seed_values[0]..seed_values[0]+seed_values[1] {
        seeds.push(value);
    }
    for value in seed_values[2]..seed_values[2]+seed_values[3] {
        seeds.push(value);
    }
    seeds
}
